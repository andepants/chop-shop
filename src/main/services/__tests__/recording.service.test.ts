...
describe('recording.service', () => {
  ...
  describe('getDefaultWebcam', () => {
    it('successfully auto-selects the first webcam', async () => {
      ...
      const result = await recordingService.getDefaultWebcam()

      expect(result).toEqual(mockDevices[0])
      expect(mockEnumerateDevices).toHaveBeenCalled()
    })

    it('throws NO_DEVICES_FOUND error when no webcams available', async () => {
      ...
      await expect(recordingService.getDefaultWebcam()).rejects.toThrow(
        new RecordingError(
    ...
    it('throws PERMISSION_DENIED error when camera permission denied', async () => {
    ...
    it('throws NO_DEVICES_FOUND error for NotFoundError', async () => {
      const notFoundError = new Error('No devices')
      notFoundError.name = 'NotFoundError'
      mockEnumerateDevices.mockRejectedValueOnce(notFoundError)
    ...
    it('throws DEVICE_BUSY error when camera is in use', async () => {
      const busyError = new Error('Device busy')
      busyError.name = 'NotReadableError'
      mockEnumerateDevices.mockRejectedValueOnce(busyError)
    ...
    it('throws UNKNOWN_ERROR for unexpected failures', async () => {
      mockEnumerateDevices.mockRejectedValueOnce(new Error('Unexpected error'))
